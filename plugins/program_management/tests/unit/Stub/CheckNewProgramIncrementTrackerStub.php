...
final class CheckNewProgramIncrementTrackerStub implements CheckNewProgramIncrementTracker
{
    ...
    public function checkProgramIncrementTrackerIsValid(
        ...
        if ($this->is_not_found) {
            throw new PlanTrackerNotFoundException($program_increment_tracker_id);
        }
        if ($this->is_not_in_program) {
            throw new PlanTrackerDoesNotBelongToProjectException($program_increment_tracker_id, $program->id);
        }
    }
}
