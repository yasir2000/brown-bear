...
class PaginationPresenter
{
    ...
    public function __construct($limit, $offset, $nb_displayed, $nb_total, $base_url, $default_params)
    {
        ...
        $this->first_url    = $base_url . '?' . http_build_query(['offset' => 0] + $default_params);
        $this->previous_url = $base_url . '?' . http_build_query(['offset' => $offset - $limit] + $default_params);
        $this->next_url     = $base_url . '?' . http_build_query(['offset' => $offset + $limit] + $default_params);
        $this->last_url     = $base_url . '?' . http_build_query(['offset' => $limit * floor($nb_total / $limit) - 1] + $default_params);
