...
foreach ($packages as $package_id => $package_for_display) {
    ...
    if ($can_see_package) {
        ...
        if (! $pv) {
            if ($permission_manager->isAdmin($project, $user)) {
                $html .= '     <a href="admin/package.php?func=edit&amp;group_id=' . $hp->purify(urlencode($group_id)) . '&amp;id=' .
                         $hp->purify(urlencode($package_id)) . '" data-test="update-package" title="' .
            ...
            $html .= '</a>';
            if ($permission_manager->isAdmin($project, $user)) {
                $html .= '     &nbsp;&nbsp;<a href="admin/package.php?func=delete&amp;group_id=' . $hp->purify(urlencode($group_id)) . '&amp;id=' . $hp->purify(urlencode($package_id)) . '" title="' .  $hp->purify($GLOBALS['Language']->getText('file_admin_editreleases', 'delete'), CODENDI_PURIFIER_CONVERT_HTML)  . '" onclick="return confirm(\'' .  $hp->purify($GLOBALS['Language']->getText('file_admin_editpackages', 'warn'), CODENDI_PURIFIER_CONVERT_HTML)  . '\');" data-test="remove-package">'
            ...
            foreach ($res_release as $package_release) {
                ...
                if ($can_see_release) {
                    ...
                    if (! $pv) {
                        if ($permission_manager->isAdmin($project, $user)) {
                            $html .= '     <a
                            href="admin/release.php?func=edit&amp;group_id=' . $hp->purify(urlencode($group_id)) . '&amp;package_id=' . $hp->purify(urlencode($package_id)) . '&amp;id=' . $hp->purify(urlencode((string) $package_release->getReleaseID())) . '"
